...
import com.zypper.modules.shiro.vo.DefContants;
import com.zypper.modules.system.entity.SysDepart;
import com.zypper.modules.system.entity.SysUser;
import com.zypper.modules.system.model.SysLoginModel;
import com.zypper.modules.system.service.ISysDepartService;
import com.zypper.modules.system.service.ISysLogService;
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel) {
		...
		if(sysUser==null) {
			...
			if(retryCount != null && Integer.valueOf(retryCount.toString()) <= 0){
				result.error500("密码错误超过3次，请30分钟后尝试");
				return result;
			}
			//2. 校验用户名或密码是否正确
			...
			if (!syspassword.equals(userpassword)) {
				Integer retryNumber = (retryCount == null) ? 3 : (Integer.valueOf(retryCount.toString()) - 1);
				...
				if (retryNumber == 0) {
					...
					result.error500("用户名或密码错误，还有" + retryNumber + "次机会");
				}
			 ...
			 * @author John
			 */
			ToFactorAuthPO authPO = authService.getOne(new QueryWrapper<ToFactorAuthPO>().lambda().eq(ToFactorAuthPO::getUsername, username));
			if (authPO != null) {
				if (pin.equals("")){
					result.error500("请输入两部验证码");
					return result;
				}
				System.out.println(authPO);
				String code = TOTP.getTOTPCode(authPO.getSecretkey());
				if (!pin.equals(code)) { // 未通过
					result.error500("两部验证码错误");
					return result;
				}
			}
